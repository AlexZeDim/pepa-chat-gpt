...
export class AppService implements OnApplicationBootstrap {
  ...
  private async loadBot(resetContext = false) {
    ...
    this.chatEngineStorage = [new OpenAIApi(config1), new OpenAIApi(config2)];
  }
  ...
  async idleReaction() {
    try {
      ...
      if (flag === PEPA_TRIGGER_FLAG.RAID_TRIGGER_HAPPY) {
        ...
        const role = await guild.roles.cache.get('543816517343641621');
        ...
        if (role && role.members.size) {
          ...
          if (
            !(
              guildMemberWithPresence.presence &&
              guildMemberWithPresence.presence.activities &&
              guildMemberWithPresence.presence.activities.length
            )
          ) {
            return;
          }
  ...
  public async process(job: Job<MessageJobInterface, MessageJobResInterface>): Promise<MessageJobResInterface> {
    try {
      ...
      const formatString = pepaNaming.test(responseChoice.text)
        ? responseChoice.text.replace(pepaNaming, '').trim()
        : responseChoice.text.trim();
