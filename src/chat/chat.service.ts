...
export class ChatService {
  ...
  async diceRollerFullHouse(
    ...
    try {
      ...
      if (!isText && !hasAttachment && !isMentioned && !isMedia) {
        if (localTime.hour() === 23 && localTime.minute() > 44 && localTime.month() === 11 && localTime.day() === 31) {
        ...
        if (localTime.weekday() === 1) {
          const [startJoinQueueMythicKey, endLeavePugsDepleteKey] = [localTime.hour(21).minute(0), localTime.hour(23).minute(59)];
        ...
        if (localTime.weekday() === 2) {
          const [startLootClownChest, endLootClownChest, startAnyGoodLoot, endAnyGoodLoot] = [
        ...
        if (localTime.weekday() < 4) {
          const [startRaidHoney, endRaidHoney] = [localTime.hour(20).minute(0), localTime.hour(23).minute(59)];
  ...
  public whoAmIContext(authorName: string): string[] {
    return [
      `${authorName}: Тебя зовут Пепа и ты любишь болтать со мной и ребятами в дискорде монахов по World of Warcraft`,
      `Пепа: Меня зовут Пепа, я люблю болтать с ребятами про World of Warcraft и помогать им.`,
  ...
  public async getLastActiveMessage(): Promise<boolean> {
    ...
    const [startRaidHoney, endRaidHoney] = [localTime.hour(9).minute(0), localTime.hour(23).minute(59)];
  ...
  public async triggerError(): Promise<string> {
    ...
    await this.redisService.set(PEPA_CHAT_KEYS.FULL_TILT_IGNORE, 1, 'EX', timeout);
